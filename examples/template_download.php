<?php

require_once __DIR__ . '/../autoload.php';

use \Ayg\Econtract\Template;

$appId      = env('APP_ID', 'openapitest');
$privateKey = env('PRIVATE_KEY', '');


$template               = new Template($appId, $privateKey);
$template->templateId   = '477';
$template->extrSystemId = 'aiyuangong';
$res                    = $template->download('./service_contract.pdf');
var_dump($res);
